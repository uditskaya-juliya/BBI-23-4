...
namespace _7._3lvl
{
    abstract class Group
    {
        ...
        public double AverageScore { get => _averageGrade; }
        public virtual void sred_rez(Group grops, ref double sr) { }

        public virtual void Print(double sredn)
        {
            Console.WriteLine("Familiya:{0,10} Potok:{1, 10} Groupa:{2, 10} sredni_rez:{3, 10}", fam, potok, group, sredn);
        }
    ...
    internal class Program
    {
        static void Main(string[] args)
        {
            ...
            for (int i = 0; i < gropsbp.Length; i++)
            {
                ...
                gropsbp[0][0].sred_rez(gropsbp[i][0], ref sredn[i]);

                for (int j = 0; j < 5; j++)
                {
                    gropsbp[i][j].Print(sredn[i]);
                }
            }
        }
        ...
        {
            QuickSort(grops, sr, 0, sr.Length - 1);
        }

        static void QuickSort(Group[][] grops, double[] sr, int low, int high)
        {
            if (low < high)
            {
                int pi = Partition(grops, sr, low, high);

                QuickSort(grops, sr, low, pi - 1);
                QuickSort(grops, sr, pi + 1, high);
            }
        }

        static int Partition(Group[][] grops, double[] sr, int low, int high)
        {
            double pivot = sr[high];
            int i = low - 1;
            for (int j = low; j < high; j++)
            {
                if (sr[j] >= pivot)
                {
                    ...
                    Group[] tempGroup = grops[i];
                    grops[i] = grops[j];
                    grops[j] = tempGroup;
                }
            }
            double tempPivot = sr[i + 1];
            sr[i + 1] = sr[high];
            sr[high] = tempPivot;
            Group[] tempGroupPivot = grops[i + 1];
            grops[i + 1] = grops[high];
            grops[high] = tempGroupPivot;
            return i + 1;
        }
    }
}
