...
class Program
{
    static void Main(string[] args)
    {
        ...
        var jsonSerializer = new JsonSerializeFormat<Group[]>();

        // Путь к файлу JSON
        string jsonFilePath = "groups.json";

        // Сериализация данных в файл
        jsonSerializer.Serialize(groups, jsonFilePath);
        Console.WriteLine("Данные сериализованы в JSON.");

        // Десериализация данных из файла
        try
        {
            Group[] deserializedJsonGroups = jsonSerializer.Deserialize(jsonFilePath);

            Console.WriteLine("\nДесериализованные данные из JSON:");
            foreach (var group in deserializedJsonGroups)
            {
                Console.WriteLine($"Фамилия: {group.fam}, Группа: {group.group}, Поток: {group.potok}, Математика: {group.math}, Программирование: {group.prog}, Физика: {group.phys}");
            }
        }
        catch (Exception ex)
        {
