...
namespace _7._2
{
    ...
    class Program
    {
        static void Main(string[] args)
        {
            ...
            xmlSerializer.Serialize(longJumpAthletes, Path.Combine(directoryPath, "longJumpAthletes.xml"));
            var deserializedXmlLongJump = xmlSerializer.Deserialize(Path.Combine(directoryPath, "longJumpAthletes.xml"));
            Console.WriteLine("XML Deserialized Long Jump:");
            foreach (var athlete in deserializedXmlLongJump)
            {
                Console.WriteLine($"{athlete.GetFamile()} {athlete.GetMaxResult()}");
            }

            xmlSerializer.Serialize(highJumpAthletes, Path.Combine(directoryPath, "highJumpAthletes.xml"));
            var deserializedXmlHighJump = xmlSerializer.Deserialize(Path.Combine(directoryPath, "highJumpAthletes.xml"));
            Console.WriteLine("XML Deserialized High Jump:");
            foreach (var athlete in deserializedXmlHighJump)
            {
                Console.WriteLine($"{athlete.GetFamile()} {athlete.GetMaxResult()}");
            }
            ...
            binarySerializer.Serialize(longJumpAthletes, Path.Combine(directoryPath, "longJumpAthletes.bin"));
            var deserializedBinaryLongJump = binarySerializer.Deserialize(Path.Combine(directoryPath, "longJumpAthletes.bin"));
            Console.WriteLine("Binary Deserialized Long Jump:");
            foreach (var athlete in deserializedBinaryLongJump)
            {
                Console.WriteLine($"{athlete.GetFamile()} {athlete.GetMaxResult()}");
            }

            binarySerializer.Serialize(highJumpAthletes, Path.Combine(directoryPath, "highJumpAthletes.bin"));
            var deserializedBinaryHighJump = binarySerializer.Deserialize(Path.Combine(directoryPath, "highJumpAthletes.bin"));
            Console.WriteLine("Binary Deserialized High Jump:");
            foreach (var athlete in deserializedBinaryHighJump)
            {
                Console.WriteLine($"{athlete.GetFamile()} {athlete.GetMaxResult()}");
            }
        }
